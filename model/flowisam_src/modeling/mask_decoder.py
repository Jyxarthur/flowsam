...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)  # B 7 256
        # Expand per-image data in batch direction to be per-mask
        if image_embeddings.shape[0] == tokens.shape[0]: ### Conditioning on whether bsz matches (i.e., train or eval)
        ...
        # Generate fIoU (foreground IoU) predictions (replacing the original iou prediction head)

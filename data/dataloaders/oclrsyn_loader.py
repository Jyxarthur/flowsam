...
class OCLRsyn_dataset(Dataset):
    ...
    def __init__(
        ...
        if "AmodalAnnotations" in data_dirs[1]:
...
class OCLRsyn_dataset_modal(Dataset):
    """
    Please use this dataloader if the amodal masks in OCLR-syn are converted to modal masks as preprocessing
        ...
        for idx, seq in enumerate(sorted(self.seqs)):
            ...
            for filename in sorted(os.listdir(seq_dir)): 
                ...
                if np.sum(ref_anno) == 0:
                    continue       
                flow_path = os.path.join(seq_dir.replace(self.anno_dir, self.flow_dir), filename)
                rgb_path = os.path.join(seq_dir.replace(self.anno_dir, self.rgb_dir), filename.replace(".png", ".jpg"))
                self.anno_paths.append(os.path.join(seq_dir, filename))
                self.flow_paths.append(flow_path)
                self.rgb_paths.append(rgb_path)
        print("Dataset consists of {} images".format(len(self.flow_paths)))
